const cityInptu = document.querySelector('.city-input')
...
const apiKey = '9335eb1d7726f5c707ddf246b50d5941'

searchBtn.addEventListener('click', () => {
    if (cityInptu.value.trim() != ''){
        updateWeatherInfo(cityInptu.value)
        cityInptu.value = ''
        cityInptu.blur()
    }
})

cityInptu.addEventListener('keydown', (event) => {
    if (event.key == 'Enter' &&
        cityInptu.value.trim() != ''
    ) {
        updateWeatherInfo(cityInptu.value)
        cityInptu.value = ''
        cityInptu.blur()
